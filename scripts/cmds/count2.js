module.exports = {
	config: {
		...
		guide: {
			vi: "   {pn}: dùng để xem số lượng tin nhắn của bạn"
				...
				+ "\n   {pn} kick: xoá tất cả thành viên chưa gửi tin nhắn",
			en: "   {pn}: used to view the number of messages of you"
				...
				+ "\n   {pn} kick: remove all members who have not sent any messages"
		}
	},
	...
	langs: {
		vi: {
			...
			kickResult: "Đã xoá %1 thành viên chưa gửi tin nhắn nào."
		},
		en: {
			...
			kickResult: "Removed %1 members who have not sent any messages."
		}
	},
	...
	onStart: async function ({ args, threadsData, message, event, api, commandName, getLang }) {
		...
		for (const user of members) {
			if (!usersInGroup.includes(user.userID))
				continue;
			const charac = "️️️️️️️️️️️️️️️️️"; // FB banned char
			arraySort.push({
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "all") {
				...
				let kickedCount = 0;
				for (const user of arraySort) {
					if (user.count === 0) {
						try {
							await api.removeUserFromGroup(user.uid, threadID);
							kickedCount++;
						} catch (e) {
							console.log(`Failed to kick ${user.uid}: ${e.message}`);
						}
					}
				}
				return message.reply(getLang("kickResult", kickedCount));
			}
